...
// --- SERVINDO ARQUIVOS ESTÁTICOS (A FORMA CORRETA E EXPLÍCITA) ---
// Quando uma URL no navegador começar com /proj, o Express procurará na pasta física 'proj'.
...
app.use(session({
    secret: 'a-chave-secreta-do-seu-projeto-pode-ser-qualquer-coisa',
    resave: false,
    saveUninitialized: true,
    cookie: {
        secure: false, // Em um site real com HTTPS, mude para 'true'
...
const writeCsv = (filePath, data) => {
    if (data.length === 0) {
        const headers = fs.readFileSync(filePath, 'utf8').split('\n')[0];
...
const isAdmin = (req, res, next) => {
    ...
    // Se não for admin, retorna erro de acesso negado
